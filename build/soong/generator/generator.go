...
func init() {
	android.RegisterModuleType("margatroid_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = margatroidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = margatroidExpandVariables(ctx, dep_file)
	...
	cmd := margatroidExpandVariables(ctx, String(g.properties.Cmd))
