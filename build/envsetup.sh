function __print_margatroid_functions_help() {
cat <<EOF
Additional MargatroidOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- margatroidgerrit:   A Git wrapper that fetches/pushes patch from/to MargatroidOS Gerrit Review.
- margatroidrebase:   Rebase a Gerrit change and push it again.
- margatroidremote:   Add git remote for MargatroidOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for MargatroidOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Margatroid model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch margatroid_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/MargatroidOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.margatroid.device | grep -q "$MARGATROID_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $MARGATROID_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function margatroidremote()
{
    ...
    git remote rm margatroid 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    local MARGATROID="true"
    if [ -z "$REMOTE" ]
    then
    ...
    then
        ...
        local PFX="MargatroidOS/"
    else
        local PROJECT=$REMOTE
    fi

    local MARGATROID_USER=$(git config --get review.review.margatroidn-messer.org.username)
    if [ -z "$MARGATROID_USER" ]
    then
        git remote add margatroid ssh://review.margatroidn-messer.org:29418/$PFX$PROJECT
    else
        git remote add margatroid ssh://$MARGATROID_USER@review.margatroidn-messer.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'margatroid' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Margatroidn-Messer/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.margatroid.device | grep -q "$MARGATROID_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        margatroidremote
        git push margatroid HEAD:refs/heads/'$1'
    fi
    '
}

function margatroidgerrit() {
    ...
    local user=`git config --get review.review.margatroidn-messer.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "margatroidgerrit" ]; then
...
function margatroidrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "MargatroidOS Gerrit Rebase Usage: "
        echo "      margatroidrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.margatroidn-messer.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.margatroid.device | grep -q "$MARGATROID_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/margatroid/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MARGATROID_FIXUP_COMMON_OUT ]; then
